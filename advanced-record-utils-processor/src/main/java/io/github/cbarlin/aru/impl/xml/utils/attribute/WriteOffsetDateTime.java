...
public final class WriteOffsetDateTime extends WriteXmlAttribute {
    ...
    public WriteOffsetDateTime(final XmlRecordHolder xmlRecordHolder, final XmlAttributePrism xmlAttributePrism, final Optional<TypeAliasComponent> typeAliasComponent) {
    ...
    void visitAttributeComponent(final AnalysedComponent analysedComponent, final XmlAttributePrism prism) {
        ...
        namespaceName.ifPresentOrElse(
            namespace -> methodBuilder.addStatement("output.writeAttribute($S, $S, val.atZoneSameInstant($T.UTC).format($T.ISO_OFFSET_DATE_TIME))", namespace, attributeName, ZONE_OFFSET, DATE_TIME_FORMATTER),
            () -> methodBuilder.addStatement("output.writeAttribute($S, val.atZoneSameInstant($T.UTC).format($T.ISO_OFFSET_DATE_TIME))", attributeName, ZONE_OFFSET, DATE_TIME_FORMATTER)
        );
