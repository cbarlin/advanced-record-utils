...
public final class WriteBoxedFloat extends WriteXmlAttribute {

    private static final TypeName TN = TypeName.FLOAT.box();

    public WriteBoxedFloat(final XmlRecordHolder xmlRecordHolder, final XmlAttributePrism xmlAttributePrism, final Optional<TypeAliasComponent> typeAliasComponent) {
    ...
    void visitAttributeComponent(final AnalysedComponent analysedComponent, final XmlAttributePrism prism) {
        ...
        namespaceName.ifPresentOrElse(
            namespace -> methodBuilder.addStatement("output.writeAttribute($S, $S, $T.valueOf(val.floatValue()))", namespace, attributeName, STRING),
            () -> methodBuilder.addStatement("output.writeAttribute($S, $T.valueOf(val.floatValue()))", attributeName, STRING)
        );
