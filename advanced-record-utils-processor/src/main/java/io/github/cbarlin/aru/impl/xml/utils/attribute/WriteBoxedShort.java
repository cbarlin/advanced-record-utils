...
public final class WriteBoxedShort extends WriteXmlAttribute {

    private static final TypeName TN = TypeName.SHORT.box();

    public WriteBoxedShort(final XmlRecordHolder xmlRecordHolder, final XmlAttributePrism xmlAttributePrism, final Optional<TypeAliasComponent> typeAliasComponent) {
    ...
    void visitAttributeComponent(final AnalysedComponent analysedComponent, final XmlAttributePrism prism) {
        ...
        namespaceName.ifPresentOrElse(
            namespace -> methodBuilder.addStatement("output.writeAttribute($S, $S, $T.valueOf(val.shortValue()))", namespace, attributeName, STRING),
            () -> methodBuilder.addStatement("output.writeAttribute($S, $T.valueOf(val.shortValue()))", attributeName, STRING)
        );
