...
@RequiresBean({ConstructorComponent.class, ComponentTargetingLibraryLoaded.class, AnalysedCollectionComponent.class})
public final class WithFluentMethodAdderLibrary extends WitherVisitor {

    private final ProcessingTarget other;

    public WithFluentMethodAdderLibrary(
            final WitherInterface witherInterface,
            final AnalysedRecord analysedRecord,
            final ComponentTargetingLibraryLoaded componentTargetingLibraryLoaded
    ) {
        super(Claims.WITHER_WITH_FLUENT, witherInterface, analysedRecord);
        this.other = componentTargetingLibraryLoaded.target();
    }
