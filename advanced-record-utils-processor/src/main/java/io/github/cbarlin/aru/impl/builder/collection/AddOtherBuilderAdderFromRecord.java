...
@RequiresBean({CollectionHandlerHelper.class, AnalysedCollectionComponent.class, ConstructorComponent.class, ComponentTargetingRecord.class})
public final class AddOtherBuilderAdderFromRecord extends CollectionRecordVisitor {

    private final AnalysedRecord target;
    private final BuilderClass builder;

    public AddOtherBuilderAdderFromRecord(
        ...
        final ComponentTargetingRecord targetingInterface
    ) {
        super(Claims.BUILDER_OTHER_BUILDER, acc.parentRecord(), acc);
    ...
    protected boolean visitCollectionComponent() {
        ...
        final ParameterSpec paramSpec = ParameterSpec.builder(otherBuilderClassName, "subBuilder", Modifier.FINAL)
        ...
        final var methodBuilder = builder.createMethod(addNameMethodName(), claimableOperation, analysedCollectionComponent, CONSUMER)
            ...
            .addStatement("return this.$L(subBuilder.$L())", addNameMethodName(), buildMethodName);
