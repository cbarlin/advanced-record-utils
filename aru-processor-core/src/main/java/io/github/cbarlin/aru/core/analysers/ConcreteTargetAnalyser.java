...
public abstract sealed class ConcreteTargetAnalyser implements TargetAnalyser permits RecordTargetAnalyser, InterfaceTargetAnalyser {
    ...
    protected void checkAndAddToPotentialTargets(final TypeName name, final String packageName, final Set<TypeElement> references) {
        if (name instanceof final ClassName className) {
            checkAndAddToPotentialTargets(packageName, className, references);
        } else if (name instanceof ParameterizedTypeName ptn) {
            for(final TypeName tn : ptn.typeArguments) {
                if (tn instanceof final ClassName cn) {
                    checkAndAddToPotentialTargets(packageName, cn, references);
                }
            }
        } else if (name instanceof final ArrayTypeName atn && atn.componentType instanceof final ClassName cname) {
            checkAndAddToPotentialTargets(packageName, cname, references);
        }
    }

    private void checkAndAddToPotentialTargets(final String packageName, final ClassName possibleTarget, final Set<TypeElement> references) {
        final TypeElement possible = APContext.elements().getTypeElement(possibleTarget.canonicalName());
        if (Objects.nonNull(possible) && !(references.contains(possible))) {
    ...
    private Optional<TypeElement> findPossibleUtilsClass(final AdvRecUtilsSettings settings) {
        ...
        final PackageElement pack = (PackageElement) packageEl;
