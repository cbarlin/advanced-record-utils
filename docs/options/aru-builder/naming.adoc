...
[%collapsible]
====
Set the prefix and suffix that's joined with the name of a collection component for "add"-er methods on the "Builder"
...
[%collapsible]
====
Set the prefix and suffix that's joined with the name of a collection component for setting the value of a time component to the current time.
...
In the above example, the generated "Builder" contains a method called `+updateDateOfBirthToPresent+` that will set the the `dateOfBirth` value to the present time:
...
In the above example, the generated "Builder" contains methods called `+metadataOfKindImplA+` and `+metadataOfKindImplB+` that only allows passing instances of that record:
...
[%collapsible]
====
Set the bridge between the name of a component and the name of an implementing class on the setter, if the component targets a collection of an interface and we know the implementing records.
...
In the above example, the generated "Builder" contains methods called `+addImplAInMetadata+` and `+addImplBInMetadata+` that only allows passing instances of that record:
