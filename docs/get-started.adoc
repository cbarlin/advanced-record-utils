...
CAUTION: At time of writing, Maven 4 has not yet been released and the pre-release version (including the associated version of the compiler plugin) contains a defect which prevents the below from working as it should (https://github.com/apache/maven-compiler-plugin/issues/1039[see this issue]). While it is deprecated, the method described under "Maven 3" above will still work under Maven 4.
...
+
--
Add the annotations and processor as dependencies with the appropriate types:
...
* https://labs.carrotsearch.com/hppc.html[https://github.com/eclipse-collections/eclipse-collections][HPPC: High Performance Primitive Collections for Java^] - List/Set Collections from HPPC are detected if referenced and handled appropriately (e.g. creating `+CharArrayList+` if requested).
** Note: `+Byte+` type collections are not supported, as there's no HPPC set version of them.
