...
@ServiceProvider
public class AddListSingleAdder extends ListRecordVisitor {

    private ClassName builderClassName;

    public AddListSingleAdder() {
        super(CommonsConstants.Claims.CORE_BUILDER_SINGLE_ITEM_ADDER);
    }

    @Override
    protected boolean visitListComponent(AnalysedCollectionComponent acc) {
        if (acc.isIntendedConstructorParam()) {
            ...
            final var method = acc.builderArtifact().createMethod(methodName, claimableOperation)
                ...
                logTrace(method, "Creating new collection as there currently isn't one");
                
                method.addStatement("this.$L(new $T<>())", name, ARRAY_LIST)
                    .endControlFlow()
                ...
                method.addStatement("this.$L(new $T<>(this.$L))", name, ARRAY_LIST, name)
                    .addStatement("this.$L.add($L)", name, name)
                .endControlFlow()
